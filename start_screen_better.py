# --- if you already added this earlier for Windows video playback, keep it ---
# import os
...
class StartScreen(Screen):
    ...
    # bind once when screen is shown; unbind when left (prevents handler stacking)
    ...
    def add_video_player(self, root):
        ...
        # fallback to Downloads (paths fixed)
        self.video_paths = [
        ...
        # (removed: Window.bind here to avoid stacking)
    ...
    def play_loop_video(self):
        # (removed: Window.unbind here; handled in on_leave)
    ...
    # idempotent: only reorders when needed; no schedule loops
    def bring_buttons_to_front(self):
        for w in (self.panel_wrapper, self.start_button, self.tutorial_button, self.keep_clicking_label):
            parent = getattr(w, "parent", None)
            if not parent:
                continue
            # children list back-to-front; last is topmost
            if parent.children and parent.children[-1] is not w:
