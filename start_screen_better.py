# --- use ffpyplayer on Windows (must be before Kivy imports) ---
import os
...
class StartScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # root container and two layers to avoid reordering thrash
        self.root = FloatLayout()
        self.bg_layer = FloatLayout()   # videos
        self.ui_layer = FloatLayout()   # buttons/labels/overlay

        self.root.add_widget(self.bg_layer)
        self.root.add_widget(self.ui_layer)
        self.add_widget(self.root)

        # overlay on UI layer
        self.fade_overlay = FadeOverlay(size_hint=(1, 1), pos_hint={"x": 0, "y": 0}, opacity_level=1)
        self.ui_layer.add_widget(self.fade_overlay)
        ...
        self.ui_layer.add_widget(self.keep_clicking_label)

        self.add_background(self.ui_layer)  # background rect behind UI (safe)
        self.add_buttons(self.ui_layer)
        self.add_video_player(self.bg_layer)

    # bind once per lifecycle (prevents handler stacking)
    ...
    def add_video_player(self, root):
        self.video_paths = [
            _os.path.join(_os.path.dirname(__file__), "m_to_nm title.mp4"),
            _os.path.join(_os.path.dirname(__file__), "m_to_nm_julian.mp4"),
            _os.path.join(_os.path.dirname(__file__), "m_to_nm_remote.mp4"),
            _os.path.join(_os.path.dirname(__file__), "m_to_nm_battery_outer.mp4"),
            _os.path.join(_os.path.dirname(__file__), "m_to_nm_battery_separator.mp4"),
            _os.path.join(_os.path.dirname(__file__), "m_to_nm_ions_still.mp4"),
            _os.path.join(_os.path.dirname(__file__), "m_to_nm_ions_moving.mp4")
        ]

        # fallback to Downloads (Windows-safe f-string)
        self.video_paths = [
            p if _os.path.exists(p) else f"C:\\Users\\Vislab Admin\\Downloads\\{_os.path.basename(p)}"
            for p in self.video_paths
        ]
        self.video_paths = [p for p in self.video_paths if _os.path.exists(p)]
    ...
    def play_intro_video(self):
        ...
        if self.video:
            ...
            self.video.unload()
            self.bg_layer.remove_widget(self.video)
        ...
        self.bg_layer.add_widget(self.video)
        Clock.schedule_once(lambda dt: setattr(self.video, 'state', 'play'), 0)
    ...
    def transition_video(self, old_video):
        ...
        if old_video:
            ...
            old_video.unload()
            self.bg_layer.remove_widget(old_video)
        ...
        self.bg_layer.add_widget(new_video)
        self.video = new_video
        Clock.schedule_once(lambda dt: setattr(new_video, 'state', 'play'), 0)
    ...
    def play_loop_video(self):
        ...
        loop_local = _os.path.join(_os.path.dirname(__file__), "fixed m to mm.mp4")
        loop_backup = "C:\\Users\\Vislab Admin\\Downloads\\fixed m to nm.mp4"
        loop_path = loop_local if _os.path.exists(loop_local) else loop_backup

        if not _os.path.exists(loop_path):
        ...
        self.bg_layer.add_widget(self.loop_video)
        Clock.schedule_once(lambda dt: setattr(self.loop_video, 'state', 'play'), 0)
    ...
    def add_buttons(self, root):
        ...
        self.ui_layer.add_widget(self.panel_wrapper)
        ...
        self.ui_layer.add_widget(self.start_button)
        self.ui_layer.add_widget(self.tutorial_button)
    ...
    def start_game(self):
        if self.loop_video:
            ...
            self.bg_layer.remove_widget(self.loop_video)
            self.loop_video = None
